			...
			if (ci == my_id) {
				if (0 != move_packet->move_time) {
					...
					if (global_delay < d_ms) global_delay++;
					   ...
					   break;
	// Add handlers for other packets to avoid "Unknown Packet" error.
	case SC_ADD_OBJECT:
...
void Worker_Thread()
{
	while (true) {
		...
		if (OP_RECV == over->event_type) {
			unsigned char* recv_buf = g_clients[ci].recv_over.IOCP_buf;
			int data_left = io_size;
			
			unsigned char* packet_buf = g_clients[ci].packet_buf;
			int prev_data = g_clients[ci].prev_packet_data;

			memcpy(packet_buf + prev_data, recv_buf, data_left);
			int total_data = prev_data + data_left;
			int processed = 0;

			while (total_data > 2) {
				unsigned short packet_size = *(reinterpret_cast<unsigned short*>(packet_buf + processed));
				if (total_data >= packet_size) {
					ProcessPacket(static_cast<int>(ci), packet_buf + processed);
			...
			if (total_data > 0) {
				memmove(packet_buf, packet_buf + processed, total_data);
			}
			g_clients[ci].prev_packet_data = total_data;

			DWORD recv_flag = 0;
			int wsarecv_ret = WSARecv(g_clients[ci].client_socket,
				&g_clients[ci].recv_over.wsabuf, 1,
				NULL, &recv_flag, &g_clients[ci].recv_over.over, NULL);
			if (SOCKET_ERROR == wsarecv_ret) {
				int err_no = WSAGetLastError();
